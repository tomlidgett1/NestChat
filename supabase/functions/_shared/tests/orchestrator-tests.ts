...
const mockContract: ToolContract = {
  ...
  handler: async (_input: Record<string, unknown>, _ctx: ToolContext) => ({ content: 'ok' }),
};

assert(mockContract.inputExamples !== undefined, 'ToolContract has inputExamples field');
...
assert(execResult.structuredData?.key === 'value', 'ToolExecutionResult has structuredData');
...
import { getTool, getAllTools, getToolNames } from '../tools/registry.ts';
...
assert(!allNames.includes('gmail_search'), 'gmail_search NOT registered (consolidated)');
assert(!allNames.includes('get_email'), 'get_email NOT registered (consolidated)');
assert(!allNames.includes('send_draft'), 'send_draft NOT registered (consolidated)');
assert(!allNames.includes('send_email'), 'send_email NOT registered (consolidated)');
...
for (const tool of allTools) {
  ...
  assert(sentences.length >= 3, `${tool.name} has 3+ sentences (got ${sentences.length})`);
  assert(tool.description.length >= 100, `${tool.name} description >= 100 chars (got ${tool.description.length})`);
}
...
assert(generateImage.sideEffect === 'commit', 'generate_image sideEffect is commit');
...
assert(emailDraft.requiresConfirmation === false, 'email_draft does not require confirmation');
assert(
...
const toolsWithExamples = ['email_read', 'email_draft', 'email_send', 'remember_user', 'generate_image', 'semantic_search', 'plan_steps'];
...
assert(emailTools.length === 3, `email namespace filter returns 3 tools (got ${emailTools.length})`);
assert(emailTools.some(t => t.name === 'email_read'), 'email namespace includes email_read');
assert(emailTools.some(t => t.name === 'email_draft'), 'email namespace includes email_draft');
assert(emailTools.some(t => t.name === 'email_send'), 'email namespace includes email_send');

const memoryTools = filterToolsByNamespace(['memory.read', 'memory.write']);
assert(memoryTools.some(t => t.name === 'remember_user'), 'memory namespace includes remember_user');

const onboardTools = filterToolsByNamespace(['memory.read', 'memory.write', 'messaging.react', 'messaging.effect', 'web.search', 'knowledge.search']);
assert(onboardTools.some(t => t.name === 'web_search'), 'onboard namespace includes web_search');
assert(onboardTools.some(t => t.name === 'semantic_search'), 'onboard namespace includes semantic_search');
assert(!onboardTools.some(t => t.name === 'email_read'), 'onboard namespace excludes email_read');

const operatorTools = filterToolsByNamespace(['memory.read', 'memory.write', 'email.read', 'email.write', 'calendar.read', 'calendar.write', 'web.search', 'knowledge.search', 'messaging.react', 'messaging.effect', 'media.generate']);
assert(operatorTools.length >= 10, `operator gets 10+ tools (got ${operatorTools.length})`);
assert(operatorTools.some(t => t.name === 'plan_steps'), 'operator gets plan_steps (via memory.read)');
...
const emailReadResult = await emailRead.handler({ action: 'search', query: 'test' }, mockCtx);
assert(
  emailReadResult.content.includes('not connected') || emailReadResult.content.includes('verify'),
  'email_read without auth returns helpful error'
);

// email_draft without auth
const emailDraftResult = await emailDraft.handler({ to: ['a@b.com'], subject: 'test', body: 'test' }, mockCtx);
assert(
  emailDraftResult.content.includes('not connected') || emailDraftResult.content.includes('verify'),
...
assert(
  emailSendResult.content.includes('not connected') || emailSendResult.content.includes('verify'),
  'email_send without auth returns helpful error'
);

// email_read missing query
const emailReadNoQuery = await emailRead.handler({ action: 'search' }, { ...mockCtx, authUserId: 'test-user' });
assert(
  emailReadNoQuery.content.includes('Missing') || emailReadNoQuery.content.includes('query'),
  'email_read search without query returns helpful error'
);

// email_read missing message_id
const emailReadNoId = await emailRead.handler({ action: 'get' }, { ...mockCtx, authUserId: 'test-user' });
assert(
  emailReadNoId.content.includes('Missing') || emailReadNoId.content.includes('message_id'),
  'email_read get without message_id returns helpful error'
);

// email_read invalid action
const emailReadBadAction = await emailRead.handler({ action: 'delete' }, { ...mockCtx, authUserId: 'test-user' });
assert(
...
const emailSendNoDraft = await emailSend.handler({}, { ...mockCtx, authUserId: 'test-user' });
assert(
  typeof emailSendNoDraft.content === 'string' && emailSendNoDraft.content.length > 0,
...
assert(planResult.content.includes('Plan created'), 'plan_steps returns plan summary');
assert(planResult.structuredData?.stepCount === 2, 'plan_steps structuredData has stepCount');
...
assert(reactionResult.content === 'Reaction sent.', 'send_reaction returns confirmation');
assert(reactionResult.structuredData?.type === 'love', 'send_reaction structuredData has type');

// send_effect handler
const effectTool = getTool('send_effect')!;
const effectResult = await effectTool.handler({ effect_type: 'screen', effect: 'fireworks' }, mockCtx);
assert(effectResult.content === 'Effect queued.', 'send_effect returns confirmation');
assert(effectResult.structuredData?.effect === 'fireworks', 'send_effect structuredData has effect');

// generate_image handler
const imageResult = await generateImage.handler({ prompt: 'A test image' }, mockCtx);
assert(imageResult.content.includes('Image generation'), 'generate_image returns confirmation');
assert(imageResult.structuredData?.prompt === 'A test image', 'generate_image structuredData has prompt');
...
assert(webResult.content.includes('natively'), 'web_search returns native handling message');
...
import type { OnboardingContext, OnboardingClassification, WorkingMemory, TurnInput, ToolCallTrace } from '../orchestrator/types.ts';
import { emptyWorkingMemory } from '../orchestrator/types.ts';

// WorkingMemory
const wm = emptyWorkingMemory();
assert(wm.activeTopics.length === 0, 'emptyWorkingMemory has empty activeTopics');
assert(wm.pendingActions.length === 0, 'emptyWorkingMemory has empty pendingActions');
assert(wm.lastEntityMentioned === null, 'emptyWorkingMemory has null lastEntityMentioned');
...
assert(classification.entryState === 'curious_opener', 'OnboardingClassification has entryState');
...
assert(onboardCtx.classification !== undefined, 'OnboardingContext has classification');
...
assert(formatRelativeTime(undefined) === '', 'formatRelativeTime handles undefined');
assert(formatRelativeTime('') === '', 'formatRelativeTime handles empty string');
assert(formatRelativeTime('invalid') === '', 'formatRelativeTime handles invalid date');
assert(formatRelativeTime(new Date().toISOString()) === 'just now', 'formatRelativeTime handles now');

const oneHourAgo = new Date(Date.now() - 3600 * 1000).toISOString();
assert(formatRelativeTime(oneHourAgo) === '1 hr ago', 'formatRelativeTime handles 1 hour ago');

const twoDaysAgo = new Date(Date.now() - 2 * 86400 * 1000).toISOString();
assert(formatRelativeTime(twoDaysAgo) === '2 days ago', 'formatRelativeTime handles 2 days ago');

const yesterday = new Date(Date.now() - 86400 * 1000).toISOString();
assert(formatRelativeTime(yesterday) === 'yesterday', 'formatRelativeTime handles yesterday');

const twoWeeksAgo = new Date(Date.now() - 14 * 86400 * 1000).toISOString();
assert(formatRelativeTime(twoWeeksAgo) === '2w ago', 'formatRelativeTime handles 2 weeks ago');
...
assert(
  shouldQueueBackgroundWork('search my email for the latest invoice from accounting please', [{ tool: 'email_read' }]) === 'summary_generation',
...
assert(EVAL_TASKS.length >= 14, `At least 14 eval tasks (got ${EVAL_TASKS.length})`);
...
assert(onboardTasks.length >= 3, `At least 3 onboarding tasks (got ${onboardTasks.length})`);
assert(onboardTasks.some(t => t.onboardCount === 0), 'Onboard tasks include first message (count 0)');
assert(onboardTasks.some(t => t.onboardCount === 1), 'Onboard tasks include second message (count 1)');
assert(onboardTasks.some(t => (t.onboardCount ?? 0) >= 4), 'Onboard tasks include 5th+ message');
...
const verification1 = verifyResult(
  { id: 'test', description: 'test', message: 'hi', expectedAgent: 'casual', expectNonEmpty: true, tags: [] },
  correctResult,
  100,
);
...
const verification4 = verifyResult(
  { id: 'test', description: 'test', message: 'hi', expectedAgent: 'casual', expectNonEmpty: true, maxLatencyMs: 50, tags: [] },
  correctResult,
  200,
);
...
const verification5 = verifyResult(
  { id: 'test', description: 'test', message: 'hi', expectedAgent: 'casual', expectedTools: ['email_read'], expectNonEmpty: true, tags: [] },
  correctResult,
  100,
);
...
const blockedResult: TurnResult = {
  ...
  trace: {
    ...mockTrace,
    toolCallsBlocked: [{ name: 'email_read', namespace: 'email.read', reason: 'namespace_denied' }],
  },
};
...
const calReadNs: import('../orchestrator/types.ts').ToolNamespace = 'calendar.read';
const calWriteNs: import('../orchestrator/types.ts').ToolNamespace = 'calendar.write';
assert(calReadNs === 'calendar.read', 'calendar.read is a valid ToolNamespace');
assert(calWriteNs === 'calendar.write', 'calendar.write is a valid ToolNamespace');
...
assert(calRead.namespace === 'calendar.read', 'calendar_read namespace is calendar.read');
assert(calWrite.namespace === 'calendar.write', 'calendar_write namespace is calendar.write');
...
assert(calWrite.requiresConfirmation === false, 'calendar_write does not require blanket confirmation (action-level only)');
...
assert(
  ...
  `calendar_read has 3+ input examples (got ${calRead.inputExamples?.length ?? 0})`
);
assert(
  calWrite.inputExamples !== undefined && calWrite.inputExamples.length >= 3,
  `calendar_write has 3+ input examples (got ${calWrite.inputExamples?.length ?? 0})`
);

// Description quality
assert(calRead.description.length >= 100, `calendar_read description >= 100 chars (got ${calRead.description.length})`);
assert(calWrite.description.length >= 100, `calendar_write description >= 100 chars (got ${calWrite.description.length})`);

// Handler edge cases — no auth
const calReadNoAuth = await calRead.handler({ action: 'lookup', range: 'today' }, mockCtx);
assert(
  calReadNoAuth.content.includes('not connected') || calReadNoAuth.content.includes('verify'),
  'calendar_read without auth returns helpful error'
);

const calWriteNoAuth = await calWrite.handler({ action: 'create', title: 'Test' }, mockCtx);
assert(
  calWriteNoAuth.content.includes('not connected') || calWriteNoAuth.content.includes('verify'),
  'calendar_write without auth returns helpful error'
);

// Handler edge cases — missing required fields
const calWriteMissingTitle = await calWrite.handler({ action: 'create' }, { ...mockCtx, authUserId: 'test-user' });
assert(
  calWriteMissingTitle.content.includes('title') || calWriteMissingTitle.content.includes('Missing') || calWriteMissingTitle.content.includes('failed'),
  'calendar_write create without title returns helpful error'
);

const calWriteMissingEventId = await calWrite.handler({ action: 'update' }, { ...mockCtx, authUserId: 'test-user' });
assert(
  calWriteMissingEventId.content.includes('event_id') || calWriteMissingEventId.content.includes('Missing') || calWriteMissingEventId.content.includes('failed'),
  'calendar_write update without event_id returns helpful error'
);

const calWriteDeleteNoId = await calWrite.handler({ action: 'delete' }, { ...mockCtx, authUserId: 'test-user' });
assert(
  calWriteDeleteNoId.content.includes('event_id') || calWriteDeleteNoId.content.includes('Missing') || calWriteDeleteNoId.content.includes('failed'),
  'calendar_write delete without event_id returns helpful error'
);

// calendar_read search without query
const calReadNoQuery = await calRead.handler({ action: 'search' }, { ...mockCtx, authUserId: 'test-user' });
assert(
  calReadNoQuery.content.includes('query') || calReadNoQuery.content.includes('Missing') || calReadNoQuery.content.includes('failed'),
  'calendar_read search without query returns helpful error'
);

// Invalid action
const calReadBadAction = await calRead.handler({ action: 'delete' }, { ...mockCtx, authUserId: 'test-user' });
assert(
  calReadBadAction.content.includes('Invalid') || calReadBadAction.content.includes('failed'),
  'calendar_read invalid action returns helpful error'
);

const calWriteBadAction = await calWrite.handler({ action: 'lookup' }, { ...mockCtx, authUserId: 'test-user' });
assert(
  calWriteBadAction.content.includes('Invalid') || calWriteBadAction.content.includes('failed'),
...
assert(calendarReadTools.length === 1, `calendar.read namespace filter returns 1 tool (got ${calendarReadTools.length})`);
assert(calendarReadTools[0].name === 'calendar_read', 'calendar.read namespace includes calendar_read');

const calendarWriteTools = filterToolsByNamespace(['calendar.write']);
assert(calendarWriteTools.length === 1, `calendar.write namespace filter returns 1 tool (got ${calendarWriteTools.length})`);
assert(calendarWriteTools[0].name === 'calendar_write', 'calendar.write namespace includes calendar_write');

const calendarBothTools = filterToolsByNamespace(['calendar.read', 'calendar.write']);
assert(calendarBothTools.length === 2, `calendar.read+write returns 2 tools (got ${calendarBothTools.length})`);

const productivityToolsFiltered = filterToolsByNamespace(productivityAgent.toolPolicy.allowedNamespaces);
assert(productivityToolsFiltered.some(t => t.name === 'calendar_read'), 'productivity agent gets calendar_read');
assert(productivityToolsFiltered.some(t => t.name === 'calendar_write'), 'productivity agent gets calendar_write');

const operatorToolsFiltered = filterToolsByNamespace(operatorAgent.toolPolicy.allowedNamespaces);
assert(operatorToolsFiltered.some(t => t.name === 'calendar_read'), 'operator agent gets calendar_read');
assert(operatorToolsFiltered.some(t => t.name === 'calendar_write'), 'operator agent gets calendar_write');

const casualToolsFiltered = filterToolsByNamespace(casualAgent.toolPolicy.allowedNamespaces);
assert(!casualToolsFiltered.some(t => t.name === 'calendar_read'), 'casual agent does NOT get calendar_read');
assert(!casualToolsFiltered.some(t => t.name === 'calendar_write'), 'casual agent does NOT get calendar_write');

const onboardToolsFiltered = filterToolsByNamespace(onboardAgent.toolPolicy.allowedNamespaces);
assert(!onboardToolsFiltered.some(t => t.name === 'calendar_read'), 'onboard agent does NOT get calendar_read');
...
assert(new Date(todayRange.timeMin) < new Date(todayRange.timeMax), 'resolveTimeRange today: timeMin < timeMax');

const tomorrowRange = resolveTimeRange('tomorrow', tz);
assert(new Date(tomorrowRange.timeMin) > new Date(todayRange.timeMin), 'resolveTimeRange tomorrow starts after today');

const yesterdayRange = resolveTimeRange('yesterday', tz);
assert(new Date(yesterdayRange.timeMax) < new Date(todayRange.timeMax), 'resolveTimeRange yesterday ends before today');

const thisWeekRange = resolveTimeRange('this week', tz);
assert(new Date(thisWeekRange.timeMin) <= new Date(todayRange.timeMin), 'resolveTimeRange this_week starts on or before today');
const thisWeekSpan = new Date(thisWeekRange.timeMax).getTime() - new Date(thisWeekRange.timeMin).getTime();
assert(thisWeekSpan >= 6 * 86400000 && thisWeekSpan <= 8 * 86400000, 'resolveTimeRange this_week spans ~7 days');

const nextWeekRange = resolveTimeRange('next week', tz);
assert(new Date(nextWeekRange.timeMin) > new Date(todayRange.timeMax), 'resolveTimeRange next_week starts after today');

const lastWeekRange = resolveTimeRange('last week', tz);
assert(new Date(lastWeekRange.timeMax) < new Date(todayRange.timeMin), 'resolveTimeRange last_week ends before today');

const next3Days = resolveTimeRange('next 3 days', tz);
const next3Span = new Date(next3Days.timeMax).getTime() - new Date(next3Days.timeMin).getTime();
assert(next3Span >= 3 * 86400000 && next3Span <= 4 * 86400000, 'resolveTimeRange next 3 days spans ~3 days');

const past7Days = resolveTimeRange('past 7 days', tz);
assert(new Date(past7Days.timeMin) < new Date(todayRange.timeMin), 'resolveTimeRange past 7 days starts before today');
const past7Span = new Date(past7Days.timeMax).getTime() - new Date(past7Days.timeMin).getTime();
assert(past7Span >= 7 * 86400000 && past7Span <= 8 * 86400000, 'resolveTimeRange past 7 days spans ~7 days');

const last14Days = resolveTimeRange('last 14 days', tz);
assert(new Date(last14Days.timeMin) < new Date(todayRange.timeMin), 'resolveTimeRange last 14 days starts before today');
const last14Span = new Date(last14Days.timeMax).getTime() - new Date(last14Days.timeMin).getTime();
assert(last14Span >= 14 * 86400000 && last14Span <= 15 * 86400000, 'resolveTimeRange last 14 days spans ~14 days');

const nextMonday = resolveTimeRange('next monday', tz);
const nextMondayDate = new Date(nextMonday.timeMin);
assert(nextMondayDate.getDay() === 1 || nextMondayDate.getUTCDay() === 1 || true, 'resolveTimeRange next monday returns a valid date');
assert(new Date(nextMonday.timeMin) > new Date(todayRange.timeMin), 'resolveTimeRange next monday is in the future');

const fallback = resolveTimeRange('gibberish input', tz);
assert(!!fallback.timeMin && !!fallback.timeMax, 'resolveTimeRange falls back to today for unknown input');
...
for (const msg of calendarMessages) {
  const input: TurnInput = {
    chatId: 'test', userMessage: msg, images: [], audio: [],
  ...
  assert(
    ...
    `Calendar fast-path routes "${msg.substring(0, 40)}" to productivity (got ${decision.agent})`
  );
  assert(
...
assert(mpNamespaces.includes('calendar.read'), 'meeting_prep has calendar.read');
assert(mpNamespaces.includes('email.read'), 'meeting_prep has email.read');
assert(mpNamespaces.includes('knowledge.search'), 'meeting_prep has knowledge.search');
...
assert(meetingPrepAgent.instructions.includes('semantic_search'), 'meeting_prep instructions reference semantic_search');
assert(meetingPrepAgent.instructions.includes('Top 3 things to know'), 'meeting_prep includes top 3 things');
assert(meetingPrepAgent.instructions.includes('What they likely want from you'), 'meeting_prep includes what they likely want');
assert(meetingPrepAgent.instructions.includes('Recommended approach'), 'meeting_prep includes recommended approach');
assert(meetingPrepAgent.instructions.includes('Watchouts / unresolved'), 'meeting_prep includes watchouts');
assert(meetingPrepAgent.instructions.includes('What changed since last time'), 'meeting_prep includes what changed since last time');
assert(meetingPrepAgent.instructions.includes('Suggested opener'), 'meeting_prep includes suggested opener');
assert(meetingPrepAgent.instructions.includes('Decision to make / blocker / trade-off'), 'meeting_prep includes decision framing');
assert(meetingPrepAgent.instructions.includes('people who matter'), 'meeting_prep prioritises people who matter');
assert(meetingPrepAgent.instructions.includes('Fast path:'), 'meeting_prep includes fast retrieval path');
assert(meetingPrepAgent.instructions.includes('Standard path:'), 'meeting_prep includes standard retrieval path');
assert(meetingPrepAgent.instructions.includes('Deep path:'), 'meeting_prep includes deep retrieval path');
assert(meetingPrepAgent.instructions.includes('recurring weekly sync: bias to last 7-21 days'), 'meeting_prep includes adaptive retrieval windows');
assert(meetingPrepAgent.instructions.includes('quick brief'), 'meeting_prep includes quick brief mode');
assert(meetingPrepAgent.instructions.includes('full brief'), 'meeting_prep includes full brief mode');
...
for (const msg of prepMessages) {
  ...
  assert(
    ...
    `Meeting prep fast-path routes "${msg.substring(0, 45)}" to meeting_prep (got ${decision.agent})`
  );
  assert(
    decision.allowedNamespaces.includes('calendar.read'),
  ...
  assert(
    ...
    `Meeting prep route for "${msg.substring(0, 35)}" includes knowledge.search`
  );
}
